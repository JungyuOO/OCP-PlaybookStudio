        ...
        for viewer_path, expected in cases:
            with self.subTest(viewer_path=viewer_path):
                ...
                if viewer_path.startswith("/wiki/figures/"):
                    self.assertIn(payload["boundary_truth"], {"official_candidate_runtime", "official_gold_playbook_runtime"})
                    self.assertTrue(str(payload["boundary_badge"] or "").strip())
    ...
    def test_viewer_document_route_falls_back_to_runtime_markdown_when_artifacts_are_missing(self) -> None:
        with self._workspace() as root:
            ...
            markdown_path.write_text(
                (
                    "# Monitoring\n\n"
                    "OpenShift Container Platform includes a preconfigured and "
                    "self-updating monitoring stack.\n"
                ),
                encoding="utf-8",
            )
